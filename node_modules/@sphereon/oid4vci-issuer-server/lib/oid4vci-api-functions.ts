...
import MongoDBClient from '../../../../src/titulacion-digital-model/src/persistence/mongodbClient'
...
export function getCredentialEndpoint<DIDDoc extends object>(
  ...
  router.post(path, async (request: Request, response: Response) => {
    try {
      ...
      const db: Db = await MongoDBClient.getInstance().connect();
      let credentialAsAny = credential.credential as any;
      await db.collection(credentialAsAny.type[credentialAsAny.type.length - 1]).insertOne(credentialAsAny);
